    ...
    : gr::sync_block("freq_counter_all_weights",
                     ...
                     gr::io_signature::makev(1, // min outputs
                                             ...
                                             { sizeof(output_type), // 5 outputs, change to 8
                                               ...
                                               sizeof(output_type),
                                               sizeof(output_type),
                                               sizeof(output_type),sizeof(output_type),sizeof(output_type),sizeof(output_type)})),
                                               d_vlen(vec_len),
                                               ...
                                               d_f_omega_w(0.0), d_omegaC(0.0), d_omegaD(0.0)
{
    ...
    for(size_t i = 0; i < d_vlen; i++)
    {
        d_omega_win[i] = (static_cast<double>(-(double(d_vlen)-1.0)/2.0  + int(i) ))*d_delta_omega; // Danielson (21)
        d_omega_winC[i] = 1.0;
                                        ...
                                        gr_vector_void_star& output_items)
{
    ...
    void freq_counter_all_weights_impl::data_arg(double *phi, const gr_complex *iq, size_t nitems, size_t offset)
    {
      // phi is always of length d_vlen, but iq is in general noutput_items * d_vlen * 2
      // so index accordingly
      for(size_t i = 0; i < nitems; i++)
      {
        phi[i] = std::atan2(static_cast<double>(iq[offset+i].imag()), static_cast<double>(iq[offset+i].real()));
      }
    }

    void freq_counter_all_weights_impl::unwrap(double *phi, double *phi_uw, size_t nitems)
    {
      ...
      //std::cout<<"n_pos_uw = " << n_pos_uw << " n_neg_uw=" << n_neg_uw << " n_neg*(api-pi) = " << n_neg_uw*(m_2pi-am_2pi) <<std::endl <<  std::flush;
    }

    int freq_counter_all_weights_impl::count_unwrap(double *phi, size_t nitems, int *n)
    {
      double d_term;
      int n_uw=0;
      n[0]=0;
      ...
      //std::cout<<"n_uw = " << n_uw << std::endl <<  std::flush;
      return n_uw;
    }
