...
try:
    from gnuradio.mikes_oot import freq_counter, double_to_float_stream, double_vector_sink
except ImportError:
    import os
    import sys
    dirname, filename = os.path.split(os.path.abspath(__file__))
    sys.path.append(os.path.join(dirname, "bindings"))
    from gnuradio.mikes_oot import freq_counter
...
class qa_freq_counter(gr_unittest.TestCase):
    ...
    def run_fixed_freq_test(self, fset, N_gates=int(pow(2,10))):
        ...
        self.counter = freq_counter((int(self.samp_rate*self.gate_s)),  # gate samples = vector length
